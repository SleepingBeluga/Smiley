...
def main():
    ...
    print("https://docs.google.com/spreadsheets/d/" + ID)
    ...
    body = {"values": values}
    write_res = sheet.values().update(spreadsheetId=ID,range="A1",valueInputOption="RAW",body=body).execute()
    # Write row headers
    ...
    write_res = sheet.values().update(spreadsheetId=ID,range="A11",valueInputOption="RAW",body=body).execute()
    ...
    range2 = {"sheetId":0,"startRowIndex":9,"endRowIndex":10,
                "startColumnIndex":1,"endColumnIndex":NUM_PLAYERS+1}
    ...
    # Execute changes
    batch_res = sheet.batchUpdate(spreadsheetId = ID,body = {"requests": requests}).execute()
